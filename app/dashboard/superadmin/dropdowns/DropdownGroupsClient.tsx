...
import {
  DropdownDetailDrawer,
  type DrawerTarget,
}                                      from '@/components/dropdowns/DropdownDetailDrawer'
...
//
// Aturan lookup verdict per item (PRESERVE dari S#124):
//   1. Entry spesifik untuk itemId ada → pakai entry itu
//   2. Tidak ada specific → cek table-level (source_id=null) → broadcast ke semua item
//   3. Tidak ada keduanya → no entry di map → useDeletePermission treat sebagai null (safe)
...
function buildSafetyMap(
  ...
  for (const grup of grupList) {
    ...
    if (specificGrup) {
      ...
      // Table-level fallback: copy verdict + count, override source_id supaya konsisten
      map[`master_dropdown_groups:${grup.id}`] = {
        ...groupsTableLevel,
        source_id: grup.id,
      }
    }
    // Else: no entry → useDeletePermission gets null → safe (0 dep default)

    // ── Option entries (per grup) ──
...
export function DropdownGroupsClient({ initialData }: Props): JSX.Element {
  ...
  // ── Dialog konfirmasi delete (hook tipis, tanpa fetch verdict) ───────────
  ...
  // ── Bulk fetch safety status → build SafetyStatusFullMap ─────────────────
  //
  // Dijalankan saat grupList berubah jumlah (load awal + setelah add/delete grup).
  // 2 fetch paralel: options + groups. Map menggabungkan keduanya dengan
  // key format `${source_table}:${source_id}` untuk lookup O(1) di useDeletePermission.
  ...
  useEffect(() => {
    ...
    async function fetchSafetyMap() {
      try {
        ...
        // Fetch gagal → biarkan map kosong → useDeletePermission treat semua sebagai safe
        // Server-side guard tetap crosscheck saat user klik hapus
      } finally {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4">
      ...
      {/* Side Peek Drawer — dibuka via kebab menu “Lihat Pemetaan” */}
      <DropdownDetailDrawer
        target={drawerTarget}
