...
// Orchestrator utama halaman Monitoring Dashboard.
// Mengelola: state sistem, SSE connection L2, polling fallback, alert rules panel.
//
// Layout 5 layer sesuai SPEC_MONITORING_DASHBOARD_v1.md BAB 6.
//
// Dibuat: Sesi #153 — PL-S09 Step 3.6
// Fix S#153: tambah type annotation eksplisit pada prev di onUpdate callback
...
export function MonitoringClient({
  ...
  // Suppress unused variable warnings — alertCount dipakai untuk future badge sidebar
  void alertCount

  // ─── SSE: subscribe ke /api/monitoring/stream ──────────────────────────────
        ...
        if (event.type === 'metric_update' && event.provider_id) {
          setSystems(prev => prev.map(sys =>
            sys.provider_id === event.provider_id
              ? {
                  ...sys,
                  status:           event.status           ?? sys.status,
                  response_time_ms: event.response_time_ms ?? sys.response_time_ms,
                  last_checked_at:  event.checked_at       ?? sys.last_checked_at,
                }
              : sys
          ))
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            ...
            {' · '}
            <span className={
              sseStatus === 'connected' ? 'text-emerald-600'
              : sseStatus === 'error'   ? 'text-red-500'
              :                           'text-amber-500'
            }>
              ●{' '}
              {sseStatus === 'connected' ? 'Realtime aktif'
               : sseStatus === 'error'   ? 'SSE error — retry 30s'
               :                           'Menghubungkan...'}
            </span>
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm hover:bg-muted/50 disabled:opacity-50"
          >
            {isRefreshing
              ? <LoadingIcon size={14} className="animate-spin" />
              : <RefreshIcon size={14} />
            }
            Refresh
          </button>
      ...
      <section>
        ...
        <div className="rounded-md border bg-muted/20 p-4 text-sm text-muted-foreground">
          ...
          <p>
            Status L1 di atas diperbarui otomatis setiap kali data baru masuk dari cron.
            Grafik time-series akan tersedia setelah data historis terkumpul.
          </p>
