...
type DialogMode = 'suspend' | 'resume' | 'terminate' | null
...
function DialogLifecycle({
  ...
  const config = {
    ...
    terminate: {
      title:       'Akhiri Tenant (Permanen)',
      borderColor: '#F09595',
      btnColor:    { bg: '#FCEBEB', text: '#A32D2D', border: '#F09595' },
      btnLabel:    'Akhiri Permanen',
      konsekuensi: [
        '⚠️ Tindakan ini TIDAK BISA dibatalkan.',
        'AdminTenant kehilangan akses permanen ke dashboard.',
        'Semua data tetap tersimpan untuk audit, tapi tidak bisa dipulihkan.',
        'Tenant tidak bisa diaktifkan kembali.',
      ],
    },
  }[mode]

  const namaMatch = konfirmasiNama.trim().toLowerCase() === tenantNama.trim().toLowerCase()
  const bisa = isResume ? true : (step === 2 && namaMatch)
          ...
          {(step === 1 || isResume) && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 500, color: '#6b7280', marginBottom: 8 }}>
                {isResume ? 'Yang akan terjadi:' : 'Langkah 1 — Baca konsekuensi:'}
              </div>
