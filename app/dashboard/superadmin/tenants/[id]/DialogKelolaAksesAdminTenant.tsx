...
// Dialog Kelola Akses AT (Mockup B4 APPROVED S#235) — orchestrator 5 state.
...
import {
  ...
  type DialogKelolaState, type AlasanCabut, type AreaAkses,
...
export function DialogKelolaAksesAdminTenant({ tenantId, tenantNama, at, onClose, onSuccess }: Props) {
  ...
  const handleSimpan = async () => {
    if (inactiveOn) {
      ...
      // Lapis 1: simpan perubahan area (placeholder — Lapis 2 akan hit API sesungguhnya)
      setState('loading')
    ...
    <div style={DB4.overlay} onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={DB4.modal}>
        ...
        <div style={DB4.ftr}>
          ...
          {inactiveOn
            ? <button onClick={handleSimpan} style={DB4.btn('0.5px solid #F09595', '#A32D2D')}>
                <i className="ti ti-user-off" /> Simpan &amp; Nonaktifkan
              </button>
            : <button onClick={handleSimpan} style={DB4.btn('0.5px solid #85B7EB', '#185FA5')}>
                <i className="ti ti-device-floppy" /> Simpan Perubahan
              </button>
          }
        </div>
      </div>
