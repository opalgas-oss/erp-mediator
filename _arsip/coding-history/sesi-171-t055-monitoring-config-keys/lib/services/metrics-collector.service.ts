...
async function collectSupabaseMetrics(creds: Record<string, string>): Promise<Record<string, unknown>> {
  ...
  return { db_active_connections: 0, db_max_connections: 60, db_size_bytes: 0, auth_requests_per_min: 0, active_sessions: 0, edge_fn_invocations: 0, edge_fn_error_rate_pct: 0, storage_used_bytes: 0, _note: 'Token dikonfigurasi — implementasi API call pending', _token_source: 'M3 Credential Management (supabase-management.access_token)' }
}

async function collectVercelMetrics(creds: Record<string, string>): Promise<Record<string, unknown>> {
  const token = creds['api_token'], projectId = creds['project_id']
  ...
  return { last_deployment_status: 'UNKNOWN', last_deployment_duration: 0, fn_invocations: 0, fn_error_rate_pct: 0, fn_duration_p50_ms: 0, fn_duration_p99_ms: 0, bandwidth_bytes: 0, _note: 'Token dikonfigurasi — implementasi API call pending', _token_source: 'M3 Credential Management (vercel.api_token)' }
}

async function collectUpstashMetrics(creds: Record<string, string>): Promise<Record<string, unknown>> {
  const restUrl = creds['rest_url'], restToken = creds['rest_token']
  ...
  try {
    const res = await fetch(`${restUrl}/info`, { headers: { 'Authorization': `Bearer ${restToken}` } })
    if (!res.ok) throw new Error(`Upstash INFO error ${res.status}`)
    return { _raw: await res.json(), _token_source: 'M3 Credential Management (upstash.rest_token)' }
  } catch (err) { return { _error: String(err), _token_source: 'M3 Credential Management (upstash.rest_token)' } }
}

async function collectCloudinaryMetrics(creds: Record<string, string>): Promise<Record<string, unknown>> {
  const cloudName = creds['cloud_name'], apiKey = creds['api_key'], apiSecret = creds['api_secret']
  ...
  return { storage_used_bytes: 0, storage_max_bytes: 0, bandwidth_bytes: 0, bandwidth_max_bytes: 0, api_calls: 0, api_calls_max: 0, _note: 'Token dikonfigurasi — implementasi API call pending', _token_source: 'M3 Credential Management (cloudinary)' }
}

async function collectGithubMetrics(creds: Record<string, string>): Promise<Record<string, unknown>> {
  const token = creds['personal_access_token'], owner = creds['repository_owner'], repo = creds['repository_name']
  ...
  try {
    const [workflowRes, prsRes] = await Promise.all([
      fetch(`https://api.github.com/repos/${owner}/${repo}/actions/runs?per_page=1`, { headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/vnd.github+json' } }),
      fetch(`https://api.github.com/repos/${owner}/${repo}/pulls?state=open&per_page=100`, { headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/vnd.github+json' } }),
    ])
    const [workflowData, prsData] = await Promise.all([workflowRes.ok ? workflowRes.json() : null, prsRes.ok ? prsRes.json() : null])
    const lastRun = workflowData?.workflow_runs?.[0]
    return { last_workflow_status: lastRun?.conclusion ?? 'UNKNOWN', last_workflow_duration: lastRun ? Math.round((new Date(lastRun.updated_at).getTime() - new Date(lastRun.created_at).getTime()) / 1000) : 0, open_pull_requests: Array.isArray(prsData) ? prsData.length : 0, last_commit_at: lastRun?.created_at ?? null, _token_source: 'M3 Credential Management (github.personal_access_token)' }
  } catch (err) { return { _error: String(err), _token_source: 'M3 Credential Management (github.personal_access_token)' } }
}
