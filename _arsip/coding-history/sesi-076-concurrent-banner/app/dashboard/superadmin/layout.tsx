// app/dashboard/superadmin/layout.tsx — ARSIP sebelum I-05 Sesi #076
...
async function fetchSidebarData(): Promise<{
  ...
  try {
    ...
    const [messages, configResult] = await Promise.all([
      ...
      db.from('config_registry').select('feature_key').is('tenant_id', null).eq('is_active', true),
    ])
    const featureKeys = [...new Set((configResult.data ?? []).map((r: { feature_key: string }) => r.feature_key))]
    return { messages: messages ?? {}, featureKeys: featureKeys.length > 0 ? featureKeys : ['security_login'] }
  } catch {
    return { messages: {}, featureKeys: ['security_login'] }
  }
}
...
export default async function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const getSidebarData = unstable_cache(fetchSidebarData, ['sidebar-data'], { revalidate, tags: ['sidebar-data'] })
