    ...
    return (
      <div className="space-y-0.5">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 py-1">
          <span className="flex-1 min-w-0 text-xs sm:text-sm font-medium text-slate-700">{item.label}</span>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <span className="text-xs text-slate-500">{item.value ? 'Aktif' : 'Tidak Aktif'}</span>
            <Switch
      ...
      <div className="space-y-0.5">
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 py-1">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
            <span className="text-xs text-slate-500">{item.enabled ? 'Aktif' : 'Tidak Aktif'}</span>
            <Switch checked={item.enabled} onCheckedChange={onEnabledToggle} className="h-4 w-8 sm:h-5 sm:w-9 data-[state=checked]:bg-blue-600 flex-shrink-0" />
            <Input type="number" value={typeof item.value === 'number' ? item.value : 0} onChange={(e) => onValueChange(Number(e.target.value))} disabled={!item.enabled} className="w-12 h-7 px-1 text-center text-xs disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed" />
            {hasUnitDropdown ? (
              <Select value={item.unit ?? ''} onValueChange={onUnitChange} disabled={!item.enabled}>
                <SelectTrigger className="h-7 w-auto min-w-fit px-2 text-xs disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"><SelectValue /></SelectTrigger>
                <SelectContent>{item.units?.map((u) => <SelectItem key={u} value={u} className="text-xs">{u}</SelectItem>)}</SelectContent>
              </Select>
            ) : (
              <span className={`text-xs w-8 flex-shrink-0 ${item.enabled ? 'text-slate-600' : 'text-slate-400'}`}>{item.unit}</span>
            )}
          </div>
        </div>
        <AdminCanChangeRow checked={item.adminCanChange} onToggle={onAdminCanChangeToggle} />
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 py-1">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
            ...
            <Select value={String(item.value)} onValueChange={onValueChange} disabled={!item.enabled}>
              <SelectTrigger className="h-7 w-auto min-w-fit px-2 text-xs disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"><SelectValue /></SelectTrigger>
              <SelectContent>{item.options?.map((opt) => <SelectItem key={opt} value={opt} className="text-xs">{opt}</SelectItem>)}</SelectContent>
            </Select>
          </div>
        </div>
        <AdminCanChangeRow checked={item.adminCanChange} onToggle={onAdminCanChangeToggle} />
  ...
  if (item.type === 'timing') {
    ...
    return (
      <div className="space-y-0.5">
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 py-1">
          <span className="flex-1 min-w-0 text-xs sm:text-sm font-medium text-slate-700">{item.label}</span>
          <TimingInput fieldName={fieldName} value={typeof item.value === 'number' ? item.value : Number(item.value)} onChange={(canonicalVal) => onValueChange(canonicalVal)} disabled={!item.enabled} />
        </div>
        <AdminCanChangeRow checked={item.adminCanChange} onToggle={onAdminCanChangeToggle} />
      </div>
    )
  ...
  if (item.type === 'json-per-role') {
    return (
      <div className="space-y-0.5 py-1">
        <span className="text-xs sm:text-sm font-medium text-slate-700 block mb-1">{item.label}</span>
        <PerRoleJsonEditor fieldName={item.fieldName ?? item.id} value={String(item.value)} valueType={item.valueType ?? 'boolean'} options={item.perRoleOptions} onChange={(jsonStr) => onValueChange(jsonStr)} disabled={!item.enabled} />
        <AdminCanChangeRow checked={item.adminCanChange} onToggle={onAdminCanChangeToggle} />
      </div>
    )
  }
