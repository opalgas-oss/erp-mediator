...
// SNAPSHOT PRE-S#127 (pre-L3.2 redo) — kondisi post-S#125 Layer 2 (comment update)
// Belum ada prop hideVerdictHeader (S#126 filesystem write gagal)
  ...
  return (
    <div className={`border rounded-lg p-4 ${style.container} ${className}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className={`text-sm font-semibold ${style.title}`}>
              Pemetaan Pemakaian{itemLabel ? ` — ${itemLabel}` : ''}
            </p>
            {!loading && data && (
        ...
        <button onClick={onClose} className="shrink-0 text-slate-400 hover:text-slate-600 transition-colors" aria-label="Tutup panel">
      ...
      {!loading && !error && data && (
        <>
          ...
          {data.dependencies.length > 0 && (
            <DependencyList dependencies={data.dependencies} onStatusChanged={handleStatusChanged} />
          )}
