...
export function DropdownOptionsPanel({ grup, onDataChanged }: Props): JSX.Element {
  ...
  const {
    deleteVerdicts, prefetchVerdicts,
    handleDeleteClick: _handleOptionDelete,
  ...
  function handleOptionDeleteClick(opsi: MasterDropdownOption) {
    _handleOptionDelete({ id: opsi.id, displayName: opsi.label })
  }

  useEffect(() => {
    const ids = opsiList.filter(o => o.is_active && !o.is_system && !o.is_default).map(o => o.id)
    if (ids.length > 0) prefetchVerdicts(ids)
