...
function validateNPWP(s: string): { ok: boolean; msg?: string } {
  const digits = s.replace(/\D/g, '')
  if (!digits) return { ok: false, msg: 'NPWP wajib diisi' }
  if (digits.length !== 15 && digits.length !== 16) {
    return { ok: false, msg: 'NPWP harus 15 digit (lama) atau 16 digit (baru NIK-linked)' }
  }
  return { ok: true }
}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                ...
                {npwpError ? (
                  ...
                  <span style={S.help}>15 digit lama atau 16 digit baru (NIK-linked)</span>
                )}
              </div>
            </div>
          </div>
