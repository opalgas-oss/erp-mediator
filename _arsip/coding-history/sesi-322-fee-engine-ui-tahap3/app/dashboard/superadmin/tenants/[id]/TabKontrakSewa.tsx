...
// ─── FeeSimulator (G27) — HARDCODE (pra-TAHAP 3) ─────────────────────────────
// ARSIP: versi ini masih hardcode komisiRate/prosesFlat/gwPercent/gwFlat
// TAHAP 3 akan mengganti dengan hitungSimulasiFee(feeData.aktif)
...
export function TabKontrakSewa({ tenant, onRefresh }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      {/* G22 — Contract Summary Banner — HARDCODE (pra-TAHAP 3) */}
      ...
      {/* Section C — HARDCODE (target rewrite TAHAP 3) */}
      <Accordion icon="ti-percentage" iconBg="#EAF3DE" iconColor="#3B6D11" title="Section C — Struktur biaya transaksi" defaultOpen
        rightContent={<button style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', fontSize: 12, borderRadius: 8, cursor: 'pointer', borderWidth: '0.5px', borderStyle: 'solid', borderColor: '#85B7EB', color: '#185FA5', background: '#E6F1FB' }}><i className="ti ti-plus" /> Tambah baris</button>}
      >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderTopWidth: '0.5px', borderTopStyle: 'solid', borderTopColor: 'rgba(0,0,0,0.12)', background: row.alt ? '#f9f9f8' : '#fff' }} onMouseEnter={e => (e.currentTarget.style.background = '#f9f9f8')} onMouseLeave={e => (e.currentTarget.style.background = row.alt ? '#f9f9f8' : '#fff')}>
                  <td style={{ padding: '9px 10px', fontWeight: 500 }}>{row.nama}</td>
                  <td style={{ padding: '9px 10px' }}><span style={{ background: row.tipeBg, color: row.tipeColor, borderWidth: '0.5px', borderStyle: 'solid', borderColor: row.tipeBorder, borderRadius: 100, padding: '2px 8px', fontSize: 11, fontWeight: 500, display: 'inline-block' }}>{row.tipe}</span></td>
                  <td style={{ padding: '9px 10px', fontWeight: 500 }}>{row.nilai}</td>
                  <td style={{ padding: '9px 10px', fontSize: 12, color: '#6b7280' }}>{row.bw}</td>
                  <td style={{ padding: '9px 10px' }}>{row.ppn === null ? <span style={{ color: '#9ca3af', fontSize: 11 }}>—</span> : (<span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 6px', borderRadius: 100, fontSize: 10, fontWeight: 500, borderWidth: '0.5px', borderStyle: 'solid', background: row.ppn ? '#EAF3DE' : '#f9f9f8', color: row.ppn ? '#3B6D11' : '#6b7280', borderColor: row.ppn ? '#97C459' : 'rgba(0,0,0,0.12)' }}>{row.ppn && <i className="ti ti-check" style={{ fontSize: 9 }} />}{row.ppn ? 'Ya' : 'Tidak'}</span>)}</td>
                  <td style={{ padding: '9px 10px', fontSize: 12, color: '#6b7280' }}>{row.tgl}</td>
                  <td style={{ padding: '9px 6px' }}>{row.ppn !== null && (<button style={{ padding: '3px 8px', fontSize: 11, borderWidth: '0.5px', borderStyle: 'solid', borderColor: 'rgba(0,0,0,0.22)', borderRadius: 8, background: 'transparent', cursor: 'pointer' }}><i className="ti ti-edit" /></button>)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={{ ...S.card, padding: '14px 16px' }}>
          ...
          <div style={{ ...S.grid2 }}>
            <FF label="Biaya yang diubah"><select style={S.input(true)}><option>Komisi platform</option><option>Biaya proses order</option></select></FF>
            ...
            <FF label="Berlaku mulai *">
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, background: '#FAEEDA', color: '#854F0B', borderWidth: '0.5px', borderStyle: 'solid', borderColor: '#EF9F27', borderRadius: 8, padding: '3px 8px', marginTop: 2 }}><i className="ti ti-info-circle" /> Tidak bisa retroaktif. Minimal H+1.</div>
            </FF>
            <FF label="Alasan perubahan (opsional)"><input type="text" placeholder="Negosiasi ulang kontrak" style={S.input(true)} /></FF>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
            <button style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, fontSize: 13, cursor: 'pointer', borderWidth: '0.5px', borderStyle: 'solid', borderColor: '#85B7EB', color: '#185FA5', background: '#E6F1FB' }}><i className="ti ti-device-floppy" /> Jadwalkan perubahan biaya</button>
          </div>
        </div>
      </Accordion>
      ...
      <Accordion icon="ti-refresh" iconBg="#FAEEDA" iconColor="#854F0B" title="Section D — Perpanjangan & penghentian">
        <div style={{ ...S.grid2, marginTop: 12 }}>
          ...
          <FF label="Periode pemberitahuan (hari)"><input type="number" value={draft.renewal_notice_days} onChange={e => set('renewal_notice_days', e.target.value)} style={S.input(true)} /></FF>
        </div>
        ...
        <div style={S.divider} />
        <div style={{ ...S.grid2 }}>
          <FF label="Denda penghentian awal (opsional)"><input type="number" value={draft.early_termination_fee} onChange={e => set('early_termination_fee', e.target.value)} placeholder="0 = tidak ada denda" style={S.input(true)} /></FF>
          <FF label="Kebijakan refund (teks)"><input type="text" value={draft.kebijakan_refund} onChange={e => set('kebijakan_refund', e.target.value)} style={S.input(true)} /></FF>
        </div>
      </Accordion>

      {/* Section E — HARDCODE (target rewrite L-08) */}
      <Accordion icon="ti-history" iconBg="#F1EFE8" iconColor="#5F5E5A" title="Section E — Riwayat perubahan biaya">
        <div style={{ fontSize: 12, color: '#6b7280', marginTop: 12, marginBottom: 14 }}>Read-only. Tersimpan minimum 36 bulan.</div>
        {[{ tgl: '1 Jan 2026', nama: 'Philips Liemena', desc: 'Biaya awal kontrak ditetapkan — Komisi 8% + Rp1.250/order', tipe: 'Awal', tipeBg: '#E6F1FB', tipeColor: '#185FA5', tipeBorder: '#85B7EB' }].map((e, i) => (
      ...
      <div style={{ position: 'sticky', bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', marginTop: 4, borderTop: '0.5px solid rgba(0,0,0,0.22)', background: 'rgba(249,249,248,0.97)', backdropFilter: 'blur(4px)' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={handleCancel} disabled={!hasChanges || saving} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, fontSize: 13, border: '0.5px solid rgba(0,0,0,0.22)', color: '#1a1a1a', background: 'transparent', cursor: (!hasChanges || saving) ? 'not-allowed' : 'pointer', opacity: (!hasChanges || saving) ? 0.5 : 1 }}>Batalkan</button>
          <button onClick={handleSave} disabled={!hasChanges || saving} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, fontSize: 13, border: '0.5px solid #85B7EB', color: '#185FA5', background: '#E6F1FB', cursor: (!hasChanges || saving) ? 'not-allowed' : 'pointer', opacity: (!hasChanges || saving) ? 0.5 : 1 }}>
