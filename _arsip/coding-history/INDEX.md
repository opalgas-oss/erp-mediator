...
| **4 Mei 2026** | **#098** | **Snapshot `sesi-098-pl-s08-m2` ditambahkan (`components/SidebarNav.tsx` sebelum tambah section KONTEN + fix label multi_role_policy — PL-S08 M2 Message Library).** |
| **4 Mei 2026** | **#099** | **Snapshot `sesi-099-sidebar-opsi-b` ditambahkan (`components/SidebarNav.tsx` sebelum refactor Opsi B: Konfigurasi + Konten keduanya collapsible button, icon SlidersHorizontal+Layers, sub-menu tanpa icon, font konsisten — PL-S08 M2 UI Fix).** |
...
| **5 Mei 2026** | **#101** | **Snapshot `sesi-101-dry-fix` ditambahkan (6 file sebelum DRY fix A-D: `lib/auth-server.ts` + `lib/utils-client.ts` + `app/api/superadmin/messages/route.ts` + `app/api/superadmin/messages/[id]/route.ts` + `app/dashboard/superadmin/messages/page.tsx` + `MessageLibraryClient.tsx` — sebelum: tambah requireSuperAdmin() shared, tambah formatDateIdShort() shared, tambah messageLibraryRepo_getAllForAdmin + MessageLibraryService_getAllForAdmin, hapus authSuperAdmin() lokal + fmtDate() lokal + query DB langsung di RSC).** |
| **5 Mei 2026** | **#105** | **Snapshot `sesi-105-fix-bug-msg-channel` ditambahkan (`MessageLibraryClient.tsx` sebelum fix BUG-MSG-CHANNEL: tambah kolom Channel + badge resolveChannelColor, fix overflow-hidden → overflow-x-auto untuk scroll horizontal).** |
| **5 Mei 2026** | **#106** | **Snapshot `sesi-106-fix-bug-dialog-tambah` ditambahkan (`MessageLibraryClient.tsx` sebelum fix BUG-DIALOG-TAMBAH: ganti Input+datalist → Select untuk field Kategori di Dialog Tambah — mengatasi React state tidak terupdate saat user pilih dari dropdown datalist).** |
| **7 Mei 2026** | **#109** | **Snapshot `sesi-109-tenant-can-override` ditambahkan (3 file sebelum refactor tenant_can_override: `app/api/config/bulk/route.ts` + `app/dashboard/superadmin/settings/security-login/page.tsx` + `app/dashboard/superadmin/settings/security-login/ConfigPageClient.tsx` — sebelum: ganti pemetaan akses_ubah → tenant_can_override di UI flow SuperAdmin. akses_ubah/akses_baca tetap dipertahankan untuk RLS).** |
