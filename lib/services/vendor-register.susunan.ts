...
export interface SusunanFormulir {
  ...
  /**
   * Katalog JENIS pola yang aktif — S#492. Ia IKUT dipulangkan, ⛔ bukan dibaca ulang oleh
   * pemakainya: layar dan server WAJIB memakai katalog yang sama persis, kalau tidak keduanya
   * bisa berbeda pendapat tentang sah atau tidaknya satu isian.
   */
...
export async function getSusunanFormulirVendor(): Promise<SusunanFormulir> {
  ...
  const [opsi, katalogPola] = await Promise.all([
    getOpsiUntukKolom(didukung),
    getKatalogPolaPublik(),
  ])
  const kolom = saringKolomYangBisaDirender(didukung, opsi)
  return { kolom, opsi, katalogPola }
}
